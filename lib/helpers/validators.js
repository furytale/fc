...
export const isTimestamp = (record, index) => {
    if (isSet(record, index) && !(new Date(record[index]).getTime() > 0)) {
...
export const notEqual = (property) => (record, index) => {
    if (isSet(record, index) && record[index] === record[property]) {
        throw new Error(`{ERRORS.VALIDATION_ERROR_NOTEQUAL}: ${index}, $(property)`);
    }
};